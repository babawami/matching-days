describe(' Matching Days ', function() {
  ...
  it("It should return class name of mon if day is Monday", function() {
    ...
    mainReg.sameDay();
    assert.equal(mainReg.colour(), 'green');
  });

  it("It should return map with day propery and days should be days of the week", function() {
    let mainReg = matchdays(); // INSTANCE
    let dayOne = mainReg.showDayOne('2018-06-04');
    let dayTwo=mainReg.showDayTwo('2018-06-05');
      assert.deepEqual( mainReg.match(dayOne, dayTwo),{day:('Monday','Tuesday')});
      // {
      //   days : mainReg.match('04-06-2018', '31-05-2018')
      // }
  });
